...
function bounce() {
  ...
  var isUp = id == "#" + selected || sqPlay.indexOf(id) >= 0;
  ...
  $(id).playKeyframe({ name: "bounce", duration: "700ms", timing: "linear" });
...
function toGradient() {
  if (inState && !changeStateOn) {
    ...
    $(".view").playKeyframe("translate 2s reverse");
    setTimeout(function () {
      $(".view").css({
        borderRadius: "100%",
        width: "0px",
...
function toSolid() {
  if (!inState && !changeStateOn) {
    ...
    open();
    setTimeout(function () {
      $(".view").playKeyframe("translate 2s", function () {
        $(".view").css({
          borderRadius: "0%",
          width: "300px",
          height: "300px",
  ...
  if (opened) {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        fill1.playKeyframe({ name: "slide3", duration: "2s" });
...
function test() {
  ...
  var body = document.getElementsByClassName("square1");
  space = (100 - 14.57726 * body.length) / (body.length * 2);
  var pos = document.getElementsByClassName("box3")[0].getBoundingClientRect();
...
function ani(id, chooseOrClear = false, num = -1) {
  ...
  if (!inState) {
    ...
    if (!chooseOrClear && selectable) {
      if (selected) {
        $("#" + selected).playKeyframe("rSelect 0.5s forwards");
      }
  ...
  if (!inState) {
    ...
    pos1 = inNum - space - sqSize * index - space * 2 * index + "%";
    inNum = (100 * (218 + 150 * num)) / 686;
    pos2 = inNum - space - sqSize * index1 - space * 2 * index1 + "%";
  } else {
    backColor(sq[0].style.backgroundColor);
    pos1 = 42.71137 - space - sqSize * index - space * 2 * index + "%";
    pos2 = 42.71137 - space - sqSize * index1 - space * 2 * index1 + "%";
  }
  ...
  if (sq1[0] && sqPlay.indexOf("#" + sq1[0].id) < 0) {
    ...
    setTimeout(function () {
      ...
      sqPlay.splice(sqPlay.indexOf("#" + sq1[0].id), 1);
    }, 400);
  }
  ...
  if (sq[0] !== sq1[0]) {
    ...
    if (!inState) {
      gradColor["color" + (num + 1)] =
        (num > 0 ? ", " : " ") + sq[0].style.backgroundColor;

      backColor({
        backgroundImage:
          gradColor.start + gradColor.color1 + gradColor.color2 + gradColor.end,
      });
